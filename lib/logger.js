...
function banner({version='unknown',node=process.version,platform=`${process.platform}/${process.arch}`,modules='scanning',database='pending',shards='pending'}={}){
  ...
  console.log('');
  console.log(`${paint(C.black,'████████████████')} ${paint(C.magenta,'REALMS')} ${paint(C.black,'████')} ${paint(C.yellow,'NETWORK')}`);
  for(let i=0;i<r.length;i++){
    const outlineR=paint(C.black,r[i]);
    const purpleR=paint(C.magenta,r[i]);
    const yellowH=paint(C.yellow,h[i]);
    const line=i===0?`${outlineR}  ${yellowH}`:`${purpleR}  ${yellowH}`;
    console.log(`${line.padEnd(42)} ${details[i]||''}`);
  }
