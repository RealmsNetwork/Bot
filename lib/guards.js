...
function preflight(interaction, command, config) {
  ...
  if (config?.permissions?.strict !== false && !interaction.member) return 'Your server membership could not be verified.';
  ...
  if (command?.nsfw === true && interaction.channel?.nsfw !== true) return 'This command can only be used in an age-restricted channel.';

  if (command?.botPermissions) {
    const missing = command.botPermissions.filter(p => !me.permissions.has(p) || (interaction.appPermissions && !interaction.appPermissions.has(p)));
