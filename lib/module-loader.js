...
function ensureModuleConfig(dir,inherited={}){const file=path.join(dir,'config.yml');let config;if(!fs.existsSync(file)){config=mergeDefaults({enabled:false,advanced:false},inherited);fs.writeFileSync(file,yaml.dump(config,{noRefs:true,lineWidth:-1}),'utf8');console.log(`[Config] Created ${file}`);}else config=readYaml(file,{enabled:false,advanced:false});if(typeof config.advanced!=='boolean'){config.advanced=false;fs.writeFileSync(file,yaml.dump(config,{noRefs:true,lineWidth:-1}),'utf8');}const advancedFile=ensureAdvanced(dir,true);const advanced=config.advanced===true?advancedFile:{};return{config,advanced};}
