...
function merge(base, extra) { const out = { ...(base || {}) }; for (const [key, value] of Object.entries(extra || {})) { if (value && typeof value === 'object' && !Array.isArray(value)) out[key] = merge(out[key], value); else if (!(key in out)) out[key] = value; } return out; }
function ensureModuleConfig(dir, inherited = {}) { const example = path.join(dir, 'config.example.yml'); const config = path.join(dir, 'config.yml'); if (!fs.existsSync(config)) { const defaults = fs.existsSync(example) ? readYaml(example) : { enabled: false }; fs.writeFileSync(config, yaml.dump(merge(inherited, defaults), { noRefs: true, lineWidth: -1 }), 'utf8'); } return fs.existsSync(config) ? readYaml(config) : {}; }
...
async function refreshCommands(client, config, silent = false) {
  ...
  for (const [name, dir] of [...moduleDirectories(moduleRoot), ...moduleDirectories(customRoot)]) {
    const dsl = loadCustomModule(dir); const local = ensureModuleConfig(dir, config[name] || {}); if (dsl?.enabled === true && local.enabled !== true) local.enabled = true; if (local.enabled !== true) continue;
    if (dsl) for (const command of buildCommands(dsl)) next.set(command.data.name, command);
    const file = path.join(dir, 'index.js'); if (fs.existsSync(file)) for (const command of collectCommands(file)) next.set(command.data.name, command);
  }
  client.commands.clear(); for (const [name, command] of next) client.commands.set(name, command); if (!silent) console.log(`[Commands] Refreshed ${client.commands.size} local commands`); return client.commands;
}
