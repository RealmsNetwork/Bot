...
function ensureModuleConfig(dir, inherited = {}) {
  ...
  if (!fs.existsSync(config)) {
    ...
    fs.writeFileSync(config, yaml.dump(merge(inherited, defaults), { noRefs: true, lineWidth: -1 }), 'utf8');
  }
  return fs.existsSync(config) ? readYaml(config) : {};
}
function collectCommands(file) {
  try { delete require.cache[require.resolve(file)]; const mod = require(file); return Array.isArray(mod?.commands) ? mod.commands.filter(x => x?.data?.name && typeof x.execute === 'function') : []; }
...
async function refreshCommands(client, config, silent = false) {
  ...
  client.commands.clear(); for (const [name, command] of next) client.commands.set(name, command);
