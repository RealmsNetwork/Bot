...
function collectCommands(file) { try { delete require.cache[require.resolve(file)]; const mod = require(file); return Array.isArray(mod?.commands) ? mod.commands.filter(x => x?.data?.name && typeof x.execute === 'function') : []; } catch (e) { console.error(`[Commands] Failed loading ${file}:`, e.message); return []; } }
function addCommands(client, commands) { for (const command of Array.isArray(commands) ? commands : []) if (command?.data?.name && typeof command.execute === 'function') client.commands.set(command.data.name, command); }
...
async function loadOne(client, name, dir, rootConfig, custom = false) {
  ...
  const inherited = rootConfig[name] || {};
  const moduleConfig = ensureModuleConfig(dir, inherited);
  if (custom && dsl?.enabled === true) moduleConfig.enabled = true;
...
async function refreshCommands(client,config,silent=false) { const next=new Map(); for(const [name,dir] of [...moduleDirectories(moduleRoot),...moduleDirectories(customRoot)]) { const dsl=loadCustomModule(dir); const local=ensureModuleConfig(dir,config[name]||{}); if(dsl?.enabled===true)local.enabled=true; if(local.enabled!==true)continue; if(dsl)for(const c of buildCommands(dsl))next.set(c.data.name,c); const file=moduleFile(dir); if(fs.existsSync(file))for(const c of collectCommands(file))next.set(c.data.name,c); } client.commands.clear(); for(const [name,c] of next)client.commands.set(name,c); if(!silent)console.log(`[Commands] Refreshed ${client.commands.size} local commands`); return client.commands; }
