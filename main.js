...
if (enabled('aichat') || enabled('automod') || enabled('logging') || enabled('leveling') || enabled('automation')) intents.push(GatewayIntentBits.GuildMessages, GatewayIntentBits.MessageContent);
...
async function deployCommands() {
  ...
  const c = config.commandDeployment || {};
  if (!c.autoDeploy || !process.env.DISCORD_CLIENT_ID || !commands.length) return;
  const rest = new REST({ version: '10' }).setToken(process.env.DISCORD_TOKEN);
  const route = c.guildOnly && config.guildId ? Routes.applicationGuildCommands(process.env.DISCORD_CLIENT_ID, config.guildId) : Routes.applicationCommands(process.env.DISCORD_CLIENT_ID);
...
async function start() {
  ...
  client.once(Events.ClientReady, async c => {
    console.log(`[Bot] Online as ${c.user.tag} | ${c.guilds.cache.size} guilds | ${client.commands.size} commands`);
  ...
  client.on(Events.InteractionCreate, async interaction => {
    ...
    if (!command) return;
    try {
      if (command.permission && !interaction.memberPermissions?.has(command.permission)) return interaction.reply({ content: 'You do not have permission to use this command.', ephemeral: true });
