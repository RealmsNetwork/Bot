...
{data:new SlashCommandBuilder().setName('invite').setDescription('Create an invite for this channel'),async execute(i){if(!i.channel.createInvite)return i.reply({content:'Invites are not supported here.',ephemeral:true});const inv=await i.channel.createInvite({maxAge:3600,maxUses:0,reason:'Bot invite command'});await i.reply(`🔗 ${inv.url}`)}},
