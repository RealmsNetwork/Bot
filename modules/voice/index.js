...
async function connect(member,client){
  ...
  if(s.connection?.joinConfig?.channelId===member.voice.channel.id&&s.connection.state.status===VoiceConnectionStatus.Ready){
  ...
  try{return await s.connectionPromise;}
  finally{if(s.connectionPromise===promise)s.connectionPromise=null;}
}
...
async function createTempRoom(member,client){
  ...
  if(!c.enabled||!c.triggerChannelId||member.voice.channelId!==c.triggerChannelId)return null;
  const guild=member.guild;
  const name=String(c.nameTemplate||"{user}'s Room").replaceAll('{user}',member.displayName).replaceAll('{username}',member.user.username).slice(0,100);
  const options={name,type:ChannelType.GuildVoice,permissionOverwrites:[]};
  if(c.categoryId)options.parent=c.categoryId;
  if(Number(c.bitrate)>0)options.bitrate=Math.min(Number(c.bitrate),Number(c.maxBitrate)||384000);
  ...
  try{
    ...
    if(c.defaultLocked)await channel.permissionOverwrites.edit(guild.roles.everyone,{Connect:false});
    if(c.defaultHidden)await channel.permissionOverwrites.edit(guild.roles.everyone,{ViewChannel:false});
