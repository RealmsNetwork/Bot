...
function shutdownChildren(signal){if(childShutdown)return;childShutdown=true;const children=manager?.shards?Array.from(manager.shards.values()):[];if(!children.length)return;boot(`Forwarding ${signal} to ${children.length} shard process(es)...`);for(const shard of children){try{shard.kill({signal});}catch(e){warn(`Could not stop shard #${shard.id}: ${e.message}`);}}}
