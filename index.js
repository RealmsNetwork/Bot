...
if(config.database?.enabled){if(config.database.primary==='mysql')packages.add('mysql2@3.24.4');if(config.database.primary==='postgres')packages.add('pg@8.23.0');if(config.database.primary==='mongodb')packages.add('mongodb@7.6.0');if(config.database.primary==='redis')packages.add('redis@6.2.1');}
...
if(config.sharding?.enabled===true){const manager=new ShardingManager(path.join(root,'main.js'),{token:process.env.DISCORD_TOKEN,totalShards:config.sharding.totalShards||'auto',shardList:config.sharding.shardList||'auto',respawn:config.sharding.respawn!==false,mode:'process'});manager.on('shardCreate',shard=>console.log(`[Shard] Spawned #${shard.id}`));manager.spawn().catch(e=>{console.error('[Shard] Spawn failed:',e);process.exit(1);});}else require('./main').start().catch(e=>{console.error('[Bootstrap] Start failed:',e);process.exit(1);});
