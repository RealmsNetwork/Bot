...
for(const dir of [...moduleDirs(path.join(root,'modules')),...moduleDirs(path.join(root,'custom-modules'))]){const manifest=readManifest(dir);if(!enabledManifest(manifest)&&!fs.existsSync(path.join(dir,'module.js'))&&!fs.existsSync(path.join(dir,'index.js')))continue;for(const spec of(Array.isArray(manifest.dependencies)?manifest.dependencies:[]))packages.add(String(spec));}
